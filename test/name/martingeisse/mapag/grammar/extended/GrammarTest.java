...
public class GrammarTest {
	...
	public void testNullTerminalDeclarations() {
		new Grammar(null, PRECEDENCE_TABLE_EMPTY, START_NONTERMINAL_NAME, PRODUCTIONS);
	}

	@Test
	public void testEmptyTerminalDeclarations() {
		// allowed by the constructor, but caught during validation
		new Grammar(ImmutableList.of(), PRECEDENCE_TABLE_EMPTY, START_NONTERMINAL_NAME, PRODUCTIONS);
	}

	@Test
	public void testNullPrecedenceTable() {
		Grammar grammar = new Grammar(TERMINALS, null, START_NONTERMINAL_NAME, PRODUCTIONS);
		Assert.assertEquals(0, grammar.getPrecedenceTable().getEntries().size());
	}

	@Test
	public void testNonemptyPrecedenceTable() {
		Grammar grammar = new Grammar(TERMINALS, PRECEDENCE_TABLE_NONEMPTY, START_NONTERMINAL_NAME, PRODUCTIONS);
		Assert.assertEquals(PRECEDENCE_TABLE_NONEMPTY, grammar.getPrecedenceTable());
	}

	@Test(expected = IllegalArgumentException.class)
	public void testNullStartNonterminalName() {
		new Grammar(TERMINALS, PRECEDENCE_TABLE_EMPTY, null, PRODUCTIONS);
	}

	@Test(expected = IllegalArgumentException.class)
	public void testEmptyStartNonterminalName() {
		new Grammar(TERMINALS, PRECEDENCE_TABLE_EMPTY, "", PRODUCTIONS);
	}

	@Test(expected = IllegalArgumentException.class)
	public void testNullProductions() {
		new Grammar(TERMINALS, PRECEDENCE_TABLE_EMPTY, START_NONTERMINAL_NAME, null);
	}

	@Test
	public void testEmptyProductions() {
		// allowed by the constructor, but caught during validation
		new Grammar(TERMINALS, PRECEDENCE_TABLE_EMPTY, START_NONTERMINAL_NAME, ImmutableList.of());
	}
